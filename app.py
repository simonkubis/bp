...
def calculate_posterior(prior, likelihood):
    ...
    # Odstraňujeme diagonálu (vzor → rovnaký vzor)
...
def find_sequence_patterns(chain, input_patterns, max_length=MAX_SEQUENCE_LENGTH):
    ...
    for p in input_lower:
        if p in chain:
            for tgt, prob in chain[p].items():
                if tgt not in visited and prob >= MEDIUM_THRESHOLD:
    ...
    # Stochastický výber prvého kroku 
    ...
    while current and len(sequence) < max_length:
        ...
        options = [
            ...
            if t not in visited and chain[current][t] >= MEDIUM_THRESHOLD
        ]
        if not options:
            break
...
st.set_page_config(layout="wide", page_title="Odhaľovanie Spoločne Použiteľných Vzorov")
st.title("Odhaľovanie Spoločne Použiteľných Vzorov")
...
with st.sidebar:
    ...
    st.markdown("**Ako používať aplikáciu:**")
    st.markdown("1. Vyberte katalóg vzorov (predvolený alebo nahrajte vlastný).")
    st.markdown("2. Kliknite 'Načítaj' na spracovanie dát.")
    st.markdown("3. Zadajte názvy vzoru/vzorov do textového poľa.")
    st.markdown("4. Pozrite si výsledky: zlúčeniny a sekvencie.")
...
if st.button("Načítaj", use_container_width=True):
    ...
    if catalog_path and Path(catalog_path).exists():
        ...
        with st.status("Prebieha analýza...", expanded=True) as status:
            ...
            pattern_names = [d["title"] for d in catalog.get("textual_descriptions", [])]
